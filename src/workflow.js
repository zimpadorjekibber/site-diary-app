...
export class Workflow {
  ...
  renderHistoryDirectory() {
    const links = `<button type="button" class="btn-secondary" data-history-kind="individual">${this.t('कारीगर को कब-कितना दिया','Worker payment history')}</button><button type="button" class="btn-secondary" data-history-kind="trade">${this.t('ट्रेड का पूरा रिकॉर्ड','Trade history')}</button>`;
    document.getElementById('timelineHistoryLinks').innerHTML = links;
    document.getElementById('historyDirectory').innerHTML = `<h3>${this.t('पुराना रिकॉर्ड देखें','View past records')}</h3><p>${this.t('किसे, किस दिन, कितना पैसा या सामान दिया — तारीख के अनुसार देखें।','See who received money or supplies, on which date, and how much.')}</p><div class="history-shortcuts">${links}</div><h4>${this.t('ट्रेड चुनें','Choose a trade')}</h4><div class="history-shortcuts">${this.store.getTrades().map(t => `<button type="button" class="btn-secondary" data-history-kind="trade" data-history-id="${esc(t.id)}">${esc(t.name)} · ${this.t('रिकॉर्ड देखें','View history')}</button>`).join('')}</div>`;
  ...
  openHistory(targetType, recipientId) {
    const people = targetType === 'trade' ? this.store.getTrades() : this.store.getWorkers();
    ...
    this.history = { targetType, recipientId: recipientId || recipients[0]?.id || '', from: '', to: '', type: '', tradeOf: tx => tx.tradeId || this.store.getWorker(tx.workerId)?.tradeId };
    document.getElementById('historyTitle').textContent = targetType === 'trade' ? this.t('ट्रेड का दिनवार खर्च — कारीगर और सांझा सामान','Trade spending by day — workers and shared supplies') : this.t('कारीगर को कब-कितना दिया','Worker payment history');
    document.getElementById('historyControls').innerHTML = `<div class="history-filters"><label>${targetType === 'trade' ? this.t('ट्रेड','Trade') : this.t('नाम','Name')}<select data-history-field="recipientId">${recipients.map(p => `<option value="${esc(p.id)}" ${p.id === this.history.recipientId ? 'selected' : ''}>${esc(p.name)}</option>`).join('')}</select></label><label>${this.t('किस तारीख से','From date')}<input type="date" data-history-field="from"></label><label>${this.t('किस तारीख तक','To date')}<input type="date" data-history-field="to"></label><label>${this.t('पैसा / सामान','Money / supplies')}<select data-history-field="type"><option value="">${this.t('सभी एंट्री','All entries')}</option>${[...new Set(['cash','ration','cylinder',...this.store.getTransactions().map(tx => tx.type)])].map(type => `<option value="${esc(type)}">${esc(getTxTypeLabel(type, this.app.currentLang))}</option>`).join('')}</select></label></div><button type="button" class="flow-link" data-history-reset>${this.t('सभी तारीखें और प्रकार दिखाएँ','Show all dates and types')}</button>`;
